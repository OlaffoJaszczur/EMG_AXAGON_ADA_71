...
Output files:
  ...
  emg_mic.csv      -- Microphone (mono, channel 0)

HARDWARE NOTE
-------------
The Axagon ADA-71 (CM106 chip) exposes a single ALSA PCM device with 2 input
channels on Linux (ICUSBAUDIO7D: USB Audio). Both Line In and Mic In share this
device; the active capture source is controlled via alsamixer.

This script records the same device twice in sequence. Use alsamixer to switch
the capture source between the two recordings if needed:
...
# ── settings ─────────────────────────────────────────────────────────────────
LINE_DEVICE = "*ICUSBAUDIO7D*"   # matches "ICUSBAUDIO7D: USB Audio (hw:4,0)"
MIC_DEVICE  = "*ICUSBAUDIO7D*"   # same physical device — source set via alsamixer
HOSTAPI     = "ALSA"
...
def _resolve_device(pattern: str, channels: int) -> tuple[int, int]:
    """Return (device_index, sample_rate) for the first matching ALSA input device."""
    ...
    raise ValueError(
        f"No {HOSTAPI} device found matching {pattern!r} with >= {channels} input channel(s).\n"
        f"Run: python -c \"import sounddevice; print(sounddevice.query_devices())\"\n"
        f"to list available devices."
    )


def _save_csv(data_1d: np.ndarray, sample_rate: int, filepath) -> None:
